...
* Install from source
  * Grab recent version from [here](https://github.com/rust-lang/mdBook/releases/latest)
    * For Ubuntu replace the `<verson>` in the following with the version from above  ```curl -s https://api.github.com/repos/rust-lang/mdBook/releases/latest | grep "mdbook-<version>-x86_64-unknown-linux-musl.tar.gz" | cut -d : -f 2,3 |tr -d \" | wget -qi -```
  * Unpack the tar into `/bin`

## Set up Default book

`mdbook init --title "Your book title" --theme`

This will configure the basic files needed to create your book and add css files to the `theme` directory.

## Install addons

We are utilizing the mdbook-admonish title found [here](https://github.com/tommilligan/mdbook-admonish)

Be sure to run a `mdbook-admonish install /src` after installation

---

## Custom configuration

Once you have all the dependancies downloaded and configured you will need to update some of the base files with the files from the [Docs-template directory](/Docs-template/)

### CSS Files

Copy the following files into the `theme/css` directory.

* [chrome.css](/Docs-template/theme/css/chrome.css)
* [general.css](/Docs-template/theme/css/general.css)
* [tabbed-code-blocks.css](/Docs-template/theme/css/tabbed-code-blocks.css)
* [variables.css](/Docs-template/theme/css/variables.css)
* [mdbook-admonish.css](/Docs-template/theme/css/mdbook-admonish.css) this file may already exist if you have installed mdbook-admonish [as show above](#install-addons)

### JS files

We have a custom JavaScript file that allows for better tabbed code blocks. You will need to create a `theme/js` directory and add [tabbed-code-blocks.js](/Docs-template/theme/js/tabbed-code-blocks.js) to this directory.

### index.hbs

There are some custom changes that need to be made to [index.hbs](/theme/index.hbs)<br>First you will need to add the following code to the `<nav id="sidebar"...>` code block to Add the Technology Services logo to the sidebar.

```hbs
<nav id="sidebar" class="sidebar" aria-label="Table of contents">
        <!-- Put TAMU logo on top of sidebar --> 
            <div class="sidebar-logo">
                <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    width="260" height="100%" viewBox="0 0 800 174.93" style="enable-background:new 0 0 800 174.93;" xml:space="preserve">
                <style type="text/css">
...
Next add the following code to set the active themes to Aggie and navy

```hbs
    <ul id="theme-list" class="theme-popup" aria-label="Themes" role="menu">
        <!--<li role="none"><button role="menuitem" class="theme" id="light">{{ theme_option "Light" }}</button></li>-->
        <li role="none"><button role="menuitem" class="theme" id="aggie">{{ theme_option "Aggie" }}</button></li>
        <!--<li role="none"><button role="menuitem" class="theme" id="rust">{{ theme_option "Rust" }}</button></li>-->
        <!--<li role="none"><button role="menuitem" class="theme" id="coal">{{ theme_option "Coal" }}</button></li>-->
        <li role="none"><button role="menuitem" class="theme" id="navy">{{ theme_option "Dark Mode" }}</button></li>
...
We configured our site to build using a [makefile](/Docs-template/makefile). This file is then referenced in our github action to run both the `mdbook init` and `mdbook build` functions then build a GitHub pages site. Addditionally you can follow the instructions found in [mdbook's GitHub wiki](https://github.com/rust-lang/mdBook/wiki/Automated-Deployment)

### Identity Security's github action
